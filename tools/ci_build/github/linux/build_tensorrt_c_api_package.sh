...
/bin/bash -c "/usr/bin/python3 /onnxruntime_src/tools/ci_build/build.py --build_dir /build --config Release --skip_tests --skip_submodule_sync --parallel --use_binskim_compliant_compile_flags --build_shared_lib --build_java --build_nodejs --use_tensorrt --cuda_version=$CUDA_VERSION --cuda_home=/usr/local/cuda-$CUDA_VERSION --cudnn_home=/usr --tensorrt_home=/usr --cmake_extra_defines 'CMAKE_CUDA_FLAGS=--generate-code=arch=compute_75,code=sm_75 --generate-code=arch=compute_80,code=compute_80 --generate-code=arch=compute_90,code=compute_90' && cd /build/Release && make install DESTDIR=/build/installed"
