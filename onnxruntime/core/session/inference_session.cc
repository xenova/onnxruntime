    ...
    if (saving_model) {
      ...
      if (saving_ort_format) {
        ...
        if (optimized_model_external_initializers_file_name.empty()) {
          ...
          const auto& prepacked_for_graph = session_state_->GetPrepackedIniitializersForGraph();
          model_saving_options.prepacked_weights_main_graph_ = (prepacked_for_graph.IsSaveModeOn())
                                                                   ? &prepacked_for_graph
                                                                   : nullptr;
