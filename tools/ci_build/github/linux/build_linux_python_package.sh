...
if [ "$BUILD_DEVICE" == "GPU" ]; then
    ...
    BUILD_ARGS+=("--use_cuda" "--use_tensorrt" "--cuda_version=$SHORT_CUDA_VERSION" "--tensorrt_home=/usr" "--cuda_home=/usr/local/cuda-$SHORT_CUDA_VERSION" "--cudnn_home=/usr/local/cuda-$SHORT_CUDA_VERSION" "--cmake_extra_defines" "CMAKE_CUDA_FLAGS=--generate-code=arch=compute_75,code=sm_75 --generate-code=arch=compute_80,code=compute_80 --generate-code=arch=compute_90,code=compute_90")
fi
